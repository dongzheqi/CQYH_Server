...
namespace 游戏服务器.副本类
{
	public static class 副本
	{
		...
		// ==== 借鉴移植(参考引擎): 魔虫窟/魔渊派对(魔源裂隙2)/魔主之地 进入创建入口(供 NPC-lua 调用) ====
		public static void 魔虫窟创建(地图实例 魔虫窟, 玩家实例 玩家, int 副本参数)
		{
			魔虫窟.关闭时间 = 主程.当前时间.AddHours(3.0);
			魔虫窟.节点计时 = 主程.当前时间.AddSeconds(3.0);
			foreach (玩家实例 current in 副本.获取在线队友(玩家))
			{
				current.玩家切换地图(魔虫窟, 地图区域类型.传送区域);
				current.修改角色变量(数字_魔虫窟每日进入次数, 计算类.时间转换(主程.当前时间.Date));
				if (current.开启七天乐)
				{
					current.修改七天进度(68, 玩家.角色数据.七天进度[68] + 1);
				}
			}
		}

		public static void 魔渊派对创建(地图实例 魔渊派对, 玩家实例 玩家)
		{
			魔渊派对.节点计时 = 主程.当前时间.AddSeconds(1.0);
			玩家.玩家切换地图(魔渊派对, 地图区域类型.未知区域, new Point(1021, 117));
		}

		public static void 魔主之地创建(地图实例 魔主之地, 玩家实例 玩家)
		{
			魔主之地.节点计时 = 主程.当前时间.AddSeconds(1.0);
			玩家.玩家切换地图(魔主之地, 地图区域类型.未知区域, new Point(914, 222));
		}
