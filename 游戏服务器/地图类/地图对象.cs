    ...
    public abstract class 地图对象
    {
        ...
        IL_1c2b:
            ...
            if (this.检查状态(游戏对象状态.护盾状态, out var buff))
            {
                ...
                {
                    // 护盾被击穿: 吸收掉的就是护盾剩余值, 剩下的才由本体承受。
                    // 原写法把两者写反了——招架伤害 取成 (技能伤害 - 护盾数值), 再 技能伤害 -= 招架伤害,
                    // 结果 技能伤害 == 护盾数值, 即「挨多重的打都只掉护盾值那么多血」, 护盾成了伤害上限器:
                    // 护盾 100 去接 BOSS 5000 的一击, 只掉 100 血。上面那条分支(护盾够厚)本身是对的,
                    // 两条分支对 招架伤害 的语义要一致 = 被护盾吃掉的量。
                    // 本分支条件为 护盾数值 <= 技能伤害, 故相减不会为负; 强转量级与原写法同界, 不引入新溢出面。
                    详情.招架伤害 = (ushort)buff.护盾数值.V;
                    详情.技能伤害 -= buff.护盾数值.V;
                    buff.护盾数值.V = 0;
