...
namespace 游戏服务器.地图类
{
	public sealed class 怪物实例 : 地图对象
	{
		...
		public 怪物实例(宠物实例 对应宠物)
		{
			...
			对应宠物.删除对象();
			// 参数原为 this(正在构造的新野怪), 应为 对应宠物。地图实例.移除对象(地图实例.cs:363) 按
			// 对象.对象类型 分派: 传 this 走 default 分支去 对象列表.Remove(新野怪) —— 既误动了新野怪,
			// 又让本该被摘掉的 对应宠物 永远留在 地图实例.宠物列表 里(地图对象.删除对象 只清全局表与网格,
			// 不碰 地图实例 的四个分类列表)。结果: 死宠物永久滞留该图宠物列表, 随叛变次数累积。
			对应宠物.当前地图?.移除对象(对应宠物);
			this.对象死亡 = false;
			base.战斗姿态 = false;
