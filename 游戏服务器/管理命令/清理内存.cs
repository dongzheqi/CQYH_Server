...
public class 清理内存 : GM命令
{
	...
	private void 执行后台清理()
	{
		// 借鉴适配: 本引擎单逻辑线程, 参照的 Task.Run 后台清理会与主循环并发改 Dictionary, 已改为同步智能清理。
		// 日志文案同步改掉 —— 原先仍写着「启动后台批处理清理…请稍后查看日志了解进度」, 而实际是当场跑完的,
		// 运维照字面理解会以为还有一个后台任务在跑, 白等或误判。
		主程.添加命令日志("<= @清理内存 [后台模式] 执行批处理清理(本引擎为单逻辑线程, 同步执行)...");
		执行智能清理();
		主程.添加命令日志("  批处理清理已完成，详见上方逐项统计");
	}
