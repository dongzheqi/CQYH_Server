...
namespace 游戏服务器.模板类
{
    ...
    public static class 假人网关
    {
        ...
        /// <summary>
        /// 假人 → 它被投放到的配额地图编号。调度是按「当前地图」统计配额的, 而假人一旦死亡,
        /// 玩家请求复活() 会把它送回重生城 —— 那里没有配额条目, 于是它既不计入任何配额、
        /// 也永远轮不到被选中下线, 而 取空闲假人() 又因它仍在 在线假人 里而不会复用它的 角色数据。
        /// 结果: 假人池只出不进, 被逐步抽干, 全都堆在主城。记下归属图以便复活后送回原图。
        /// </summary>
        private static readonly Dictionary<玩家实例, int> 假人归属图 = new Dictionary<玩家实例, int>();
        ...
        /// <summary>取假人的归属图编号; 没有记录时返回 -1(必然与任何真实地图编号不等, 触发送回/下线兜底)。</summary>
        private static int 查归属编号(玩家实例 假人)
        {
            return 假人归属图.TryGetValue(假人, out int 编号) ? 编号 : -1;
        }
        ...
        /// <summary>
        /// 把离开了归属图的假人送回去(复活后被丢回重生城、或被 NPC/脚本传走)。
        /// 归属图已不存在时直接下线, 让 角色数据 回到池子由 调度上下线 重新投放, 避免假人卡在无配额的图上。
        /// </summary>
        private static void 送回归属图(玩家实例 假人)
        {
            if (!假人归属图.TryGetValue(假人, out int 归属编号))
            {
                假人下线(假人);
                return;
            }
            地图实例 图 = 地图处理网关.已分配地图(归属编号);
            if (图 == null)
            {
                假人下线(假人);
                return;
        ...
        private static void 驱动假人行为(玩家实例 假人)
        {
            ...
            // 复活会把假人送到重生地图(主城), 那里没有配额条目, 必须再送回归属图, 否则它会永远滞留:
            // 调度按当前地图统计配额, 滞留的假人不计入任何配额、也永远不会被选中下线, 池子只出不进。
            if (假人.对象死亡)
            {
                假人.玩家请求复活();
                if (假人.当前地图 == null || 假人.当前地图.地图编号 != 查归属编号(假人))
                {
                    送回归属图(假人);
                }
                return;
            }
            // 兜底: 任何其它原因(NPC 传送、脚本换图)离开归属图, 同样送回, 避免卡在无配额的图上。
            if (假人.当前地图.地图编号 != 查归属编号(假人))
            {
                送回归属图(假人);
                return;
            }
