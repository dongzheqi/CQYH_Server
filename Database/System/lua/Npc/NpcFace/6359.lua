        ...
        if 玩家.所属队伍 ~= nil and 玩家.所属队伍.队长编号 == 玩家.地图编号 then -- 检测是不是队长
            ...
            if 所需物品 == nil then
                ...
                for key, value in pairs(玩家列表) do
                    local 队员 = value
                    -- 原为 玩家.角色数据...: 循环里逐个队员检查, 却每次都只读队长自己的计数,
                    -- 于是非队长队员的每日次数限制完全失效(轮流当队长即可无限进)。
                    if 队员.角色数据.脚本变量[数字_未知二层每日进入次数] == tonumber(os.date("%j")) then
                ...
                if 未知暗殿 ~= nil then
                    ...
                    for key, value in pairs(玩家列表) do
                        local 队员 = value
                        -- 同上: 原来只把队长的计数写成今天, 被带进来的队员计数从未写入, 次日之前可反复进。
                        队员.角色数据.脚本变量[数字_未知二层每日进入次数] = tonumber(os.date("%j"))
